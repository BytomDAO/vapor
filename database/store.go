...
func (s *Store) SaveChainStatus(node, irreversibleNode *state.BlockNode, view *state.UtxoViewpoint, vrs []*state.VoteResult) error {
	batch := s.db.NewBatch()
	if err := saveUtxoView(batch, view); err != nil {
		return err
	}

	if err := saveVoteResult(batch, vrs); err != nil {
		return err
	}
...
func saveVoteResult(batch dbm.Batch, vrs []*state.VoteResult) error {
	for _, vr := range vrs {
		...
		batch.Set(calcVoteResultKey(vr.Seq), bytes)
	}
	return nil
}
