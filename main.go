...
func TestDSSSignature(t *testing.T) {
    ...
    assert.Nil(t, err)
    // kyber自带的门限schnorr验签操作
    assert.Nil(t, VerifyS(longterms[0].Public(), dss0.msg, buff))

    ppdata, pperr := longterms[0].Public().MarshalBinary()
    fmt.Println(longterms[0].Public())
    fmt.Println(pperr)
    fmt.Println(ppdata)
    hppdata := hex.EncodeToString(ppdata)
    fmt.Println(hppdata)
    //assert.Nil(t, Verify(ppdata, dss0.msg, buff))
    // 用Bytom的ed25519签名进行验证
    if(Verify(ppdata, dss0.msg, buff)) {
        fmt.Println("success")
    }
}
