...
func TestRollback(t *testing.T) {
	...
	for caseIndex, c := range cases {
		...
		expectBestConsensuReuslt, err := chain.GetConsensusResultByHash(bestBlockHash)
		...
		nowBestBlockHash := chain.BestBlockHash()
		nowBestConsensuReuslt, err := chain.GetConsensusResultByHash(nowBestBlockHash)
		if !testutil.DeepEqual(expectBestConsensuReuslt, nowBestConsensuReuslt) {
			t.Fatalf("%s test failed, expected: %v, now: %v", c.desc, expectBestConsensuReuslt, nowBestConsensuReuslt)
		}
