...
func TestEstimateTxGas(t *testing.T) {
	...
	for i, c := range cases {
		...
		gotActualTotalNeu := estimateTxGasResp.FlexibleNeu + estimateTxGasResp.VMNeu + estimateTxGasResp.StorageNeu
		if gotActualTotalNeu != c.actualTotalNeu {
			t.Errorf(`case#%d, got TotalNeu =%#v; want=%#v`, i, gotActualTotalNeu, c.wantTotalNeu)
		}
