			...
			wantMatchedTxs: []*types.TxData{
				{
					...
					Outputs: []*types.TxOutput{
						...
						types.NewIntraChainOutput(*orders[0].ToAssetID, 10, nil),
					},
				},
			},
		},
			...
			wantMatchedTxs: []*types.TxData{
				{
					...
					Outputs: []*types.TxOutput{
						...
						// fee
						types.NewIntraChainOutput(*orders[4].FromAssetID, 40, nil),
					},
				},
				{
					Inputs: []*types.TxInput{
					...
					Outputs: []*types.TxOutput{
						...
						types.NewIntraChainOutput(*orders[1].FromAssetID, 1, nil),
					},
				},
			},
		},
	...
	for i, c := range cases {
		...
		for i, gotMatchedTx := range gotMatchedTxs {
			...
			if gotMatchedTx.ID != wantMatchedTx.ID {
				gotTx, err := gotMatchedTx.MarshalText()
				if err != nil {
					t.Fatal(err)
				}

				wantTx, err := wantMatchedTx.MarshalText()
				if err != nil {
					t.Fatal(err)
				}
				t.Errorf("#%d(%s) the tx hash of got matched tx: %s is not equals want matched tx: %s", i, c.desc, hex.EncodeToString(gotTx), hex.EncodeToString(wantTx))
			}
		}
	}
}
