...
func buildOrderTable(store database.MovStore, txs []*types.Tx) (*match.OrderTable, error) {
	...
	var extraAddOrders, extraDelOrders []*common.Order
	for _, tx := range nonMatchedTxs {
		...
		extraAddOrders = append(extraAddOrders, addOrders...)
		extraDelOrders = append(extraDelOrders, delOrders...)
	}

	return match.NewOrderTable(store, extraAddOrders, extraDelOrders), nil
}
