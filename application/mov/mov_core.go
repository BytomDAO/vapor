...
func (m *Core) IsDust(tx *types.Tx) bool {
	for _, input := range tx.Inputs {
		if contract.IsP2WMCScript(input.ControlProgram()) && !contract.IsCancelClauseSelector(input) {
			return true
		}
	}
	return false
...
func (m *Core) ValidateTx(tx *types.Tx, verifyResult *bc.TxVerifyResult, blockHeight uint64) error {
	...
	for _, output := range tx.Outputs {
		if !contract.IsP2WMCScript(output.ControlProgram()) {
			continue
		}
...
func calcFeeAmount(matchedTx *types.Tx) (map[bc.AssetID]*matchedTxFee, error) {
	...
	for _, output := range matchedTx.Outputs {
		...
		if _, ok := dealProgMaps[hex.EncodeToString(output.ControlProgram())]; ok || contract.IsP2WMCScript(output.ControlProgram()) {
...
func validateCancelOrderTx(tx *types.Tx) error {
	for _, input := range tx.Inputs {
		if !contract.IsP2WMCScript(input.ControlProgram()) {
...
func parseAddOrdersFromTx(tx *Tx) ([]*common.Order, error) {
	...
	for i, output := range tx.rawTx.Outputs {
		if output.OutputType() != types.IntraChainOutputType || !contract.IsP2WMCScript(output.ControlProgram()) {
			continue
		}
...
func parseDeleteOrdersFromTx(tx *types.Tx) ([]*common.Order, error) {
	var orders []*common.Order
	for i, input := range tx.Inputs {
		if input.InputType() != types.SpendInputType || !contract.IsP2WMCScript(input.ControlProgram()) {
			continue
		}
