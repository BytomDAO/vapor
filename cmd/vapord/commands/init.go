...
func initFiles(cmd *cobra.Command, args []string) {
	...
	if _, err := os.Stat(fedFilePath); os.IsNotExist(err) {
		if err := cfg.ExportFederationFile(fedFilePath, config); err != nil {
			log.WithFields(log.Fields{"module": logModule, "config": fedFilePath, "error": err}).Info("fail on export federation file")
			return
		}
	}
	...
	if _, err := os.Stat(keyFilePath); os.IsNotExist(err) {
		...
		if err != nil {
			log.WithFields(log.Fields{"module": logModule, "err": err}).Info("fail on generate private key")
			return
		}

		if err := ioutil.WriteFile(keyFilePath, []byte(hex.EncodeToString(xprv[:])), 0600); err != nil {
			log.WithFields(log.Fields{"module": logModule, "err": err}).Info("fail on save private key")
			return
		}
